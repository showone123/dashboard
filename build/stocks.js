(function () {
  ...
  function draw(items) {
    ...
    codes.forEach(function (code) {
      ...
      cell(row, code); cell(row, number(item.last_price, 2));
  ...
  function open(node, userId) {
    ...
    root.innerHTML = '<div class="stock-heading"><div><small>STOCK WORKSPACE</small><h1>股票工作台</h1><p>A 股自选行情快照</p></div><button class="btn primary" type="button" data-stock-refresh>刷新</button></div>' +
      ...
      '<p class="stock-status" data-stock-status aria-live="polite">正在加载…</p><div class="stock-table-wrap"><table class="tbl"><thead><tr><th>代码</th><th>最新价</th><th>涨跌</th><th>涨跌幅</th><th>成交量</th><th>成交额</th><th>操作</th></tr></thead><tbody></tbody></table></div>' +
